...
const Leaderboard = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${statusColors[status] || "bg-gray-100 text-gray-700 border-gray-200"}`}>
        {status ? status.charAt(0).toUpperCase() + status.slice(1) : "N/A"}
      </span>
    );
  };
  ...
  const parseExtractedResume = (resumeText) => {
    if (!resumeText) return null;
    const sections = {
      personalDetails: [], skills: [], experience: [], education: [], certifications: [], projects: [], achievements: [],
    };
    const lines = resumeText.split('\n');
    let currentSection = '';
    lines.forEach((line) => {
      ...
      if (trimmedLine.startsWith('### Personal Details')) {
        ...
        if (line.startsWith('- ')) {
          ...
          if (sections[currentSection].length > 0) {
            sections[currentSection][sections[currentSection].length - 1] += '\n' + trimmedLine;
          }
        } else if (trimmedLine.startsWith('-') && currentSection !== 'projects' && currentSection !== 'experience') {
          sections[currentSection].push(trimmedLine.substring(1).trim());
        } else if (currentSection && trimmedLine && !trimmedLine.startsWith('###')) {
  ...
  const getScoreHistogram = () => {
    const bins = [0, 20, 40, 60, 80, 100];
    const labels = bins.slice(0, -1).map((start, i) => `${start}-${bins[i+1]}`);
  ...
  const statusBreakdownData = {
    ...
    datasets: [{
      ...
      backgroundColor: ['rgba(34, 197, 94, 0.6)', 'rgba(59, 130, 246, 0.6)', 'rgba(156, 163, 175, 0.6)', 'rgba(239, 68, 68, 0.6)', 'rgba(239, 68, 68, 0.6)'],
      borderColor: ['rgba(34, 197, 94, 1)', 'rgba(59, 130, 246, 1)', 'rgba(156, 163, 175, 1)', 'rgba(239, 68, 68, 1)', 'rgba(239, 68, 68, 1)'],
      borderWidth: 1,
    }],
  };
  ...
  const averageMetricsData = {
    ...
    datasets: [{
      ...
      data: [
        leaderboardData.reduce((sum, c) => sum + (c.Devscore || 0), 0) / (leaderboardData.length || 1),
        leaderboardData.reduce((sum, c) => sum + (c.Resumescore || 0), 0) / (leaderboardData.length || 1),
        leaderboardData.reduce((sum, c) => sum + (c.DsaScore || 0), 0) / (leaderboardData.length || 1),
      ],
  ...
  const topCandidates = [...leaderboardData]
    .sort((a, b) => getEffectiveScore(b) - getEffectiveScore(a))
    .slice(0, 5);

  const radarChartData = {
    labels: ['Devscore', 'Resumescore', 'DsaScore'],
    datasets: topCandidates.map((candidate, index) => ({
      label: candidate.Application?.user?.username || `Candidate ${index + 1}`,
      data: [candidate.Devscore || 0, candidate.Resumescore || 0, candidate.DsaScore || 0],
  ...
  const cumulativeScoreData = {
    labels: topCandidates.map((c, index) => `Rank ${index + 1}`),
    datasets: [
      { label: 'Dev Score', data: topCandidates.map(c => c.Devscore || 0), borderColor: 'rgba(255, 99, 132, 1)', backgroundColor: 'rgba(255, 99, 132, 0.2)', fill: true },
      { label: 'Resume Score', data: topCandidates.map(c => c.Resumescore || 0), borderColor: 'rgba(54, 162, 235, 1)', backgroundColor: 'rgba(54, 162, 235, 0.2)', fill: true },
      { label: 'DSA Score', data: topCandidates.map(c => c.DsaScore || 0), borderColor: 'rgba(75, 192, 192, 1)', backgroundColor: 'rgba(75, 192, 192, 0.2)', fill: true },
    ],
  };
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className={`transform transition-all duration-1000 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-600 to-indigo-600 animate-shimmer"></div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">Comprehensive candidate performance overview</p>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 bg-purple-50 rounded-lg">
                  <p className="text-sm font-medium text-purple-600">Total Candidates</p>
                ...
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{Math.max(...sortedData.map((c) => getEffectiveScore(c))).toFixed(1)}</p>
                </div>
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm font-medium text-blue-600">Completed</p>
                  <p className="text-2xl font-bold text-gray-900">{sortedData.filter((c) => c.status === "completed").length}/{sortedData.length}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Leaderboard Table */}
        <div className={`mt-8 transform transition-all duration-1000 delay-200 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            {sortedData.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Rank</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Candidate</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Overall Score</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Dev Score</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Resume Score</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">DSA Score</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Status</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Start Time</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-purple-600 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {sortedData.map((candidate, index) => (
                      <tr key={candidate.id} className={`hover:bg-gray-50 transition-colors ${index < 3 ? "bg-purple-50" : ""}`}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {getRankIcon(index + 1)}
                            <span className="ml-2 text-sm font-bold text-gray-900">#{index + 1}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div className="ml-4">
                              <div className="text-sm font-medium text-gray-900">{candidate.Application?.user?.username || "Anonymous"}</div>
                              <div className="text-xs text-gray-500">ID: {candidate.Application?.id}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`inline-flex px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(getEffectiveScore(candidate))}`}>
                            {getEffectiveScore(candidate).toFixed(1)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`inline-flex px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(candidate.Devscore)}`}>
                            {typeof candidate.Devscore === 'number' ? candidate.Devscore.toFixed(1) : "N/A"}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`inline-flex px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(candidate.Resumescore)}`}>
                            {typeof candidate.Resumescore === 'number' ? candidate.Resumescore.toFixed(1) : "N/A"}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`inline-flex px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(candidate.DsaScore)}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex gap-2">
                            <button
                              onClick={() => handleViewDetails(candidate)}
                              className="text-blue-600 hover:text-blue-800 flex items-center gap-1 hover:bg-blue-50 px-3 py-1 rounded-lg transition-all border border-blue-200 hover:border-blue-300"
                            >
                              <Eye className="w-4 h-4" /> Interview
                            </button>
                            <button
                              onClick={() => handleViewApplication(candidate.Application)}
                              className="text-green-600 hover:text-green-800 flex items-center gap-1 hover:bg-green-50 px-3 py-1 rounded-lg transition-all border border-green-200 hover:border-green-300"
                            >
        ...
        {sortedData.length > 0 && (
          <div className={`mt-8 transform transition-all duration-1000 delay-400 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-indigo-600" />
                Top Candidates Score Trends
              </h3>
              <Line
                data={cumulativeScoreData}
                options={{
                  responsive: true,
                  plugins: { legend: { position: 'top' }, title: { display: true, text: 'Score Trends for Top Candidates', font: { size: 16 } } },
                  scales: { y: { beginAtZero: true, max: 100 } },
                }}
                height={300}
              />
        ...
        {/* Supporting Charts */}
        {sortedData.length > 0 && (
          <div className={`mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 transform transition-all duration-1000 delay-600 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              ...
              <Bar
                data={getScoreHistogram()}
                options={{
                  responsive: true,
                  plugins: { legend: { display: false }, title: { display: true, text: 'Score Ranges' } },
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <Pie className="w-5 h-5 text-blue-600" />
                Status Breakdown
              </h3>
              <Pie
                data={statusBreakdownData}
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
              ...
              <Radar
                ...
                options={{
                  responsive: true,
                  plugins: { legend: { position: 'top' }, title: { display: true, text: 'Top Candidate Performance' } },
        ...
        {showHistory && selectedCandidate && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden border border-gray-200 shadow-xl">
              <div className="sticky top-0 bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-4 flex justify-between items-center border-b border-gray-200">
                ...
                <button
                  onClick={() => setShowHistory(false)}
                  className="text-gray-500 hover:text-gray-700 text-xl font-bold hover:bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center"
                >
                  ✕
                </button>
              </div>
              <div className="bg-gray-50 px-6 py-3 border-b border-gray-200">
                <div className="flex gap-2">
                  {['overview', 'questions', 'dsa', 'resume_conversations'].map((tab) => (
                    ...
                    >
                      {tab === 'overview' && <LayoutDashboard className="w-4 h-4 inline mr-1" />}
                      {tab === 'questions' && <MessageSquare className="w-4 h-4 inline mr-1" />}
                      {tab === 'dsa' && <FileCode className="w-4 h-4 inline mr-1" />}
                      {tab === 'resume_conversations' && <BarChart2 className="w-4 h-4 inline mr-1" />}
                      {tab.charAt(0).toUpperCase() + tab.slice(1).replace('_', ' ')}
                    </button>
                  ))}
                </div>
                    ...
                    <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                          ...
                          <div className={`px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(getEffectiveScore(selectedCandidate))}`}>
                        ...
                        <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                          ...
                          <p className="text-sm font-medium text-gray-900">
                            {selectedCandidate.start_time && selectedCandidate.end_time
                              ? `${Math.round((new Date(selectedCandidate.end_time) - new Date(selectedCandidate.start_time)) / (1000 * 60))} min`
                              : "In Progress"}
                          </p>
                        </div>
                      </div>
                      ...
                      selectedCandidate.session.map((item, index) => (
                        <div key={index} className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                          ...
                          <div className="space-y-4">
                            <div>
                              <p className="text-sm font-medium text-purple-600 mb-1">Main Question:</p>
                              <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{item.Customquestion?.question || "N/A"}</p>
                            </div>
                            {item.Customquestion?.answer && (
                              <div>
                                <p className="text-sm font-medium text-purple-600 mb-1">Expected Answer:</p>
                                <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{item.Customquestion.answer}</p>
                              </div>
                            )}
                            {item.followups && item.followups.length > 0 && (
                              <div>
                                <p className="text-sm font-medium text-purple-600 mb-2">Follow-up Questions:</p>
                                {item.followups.map((qa, qaIndex) => (
                                  <div key={qaIndex} className="bg-gray-50 p-4 rounded-md border border-gray-200 mb-2">
                                    <p className="text-sm font-medium text-blue-600 mb-1">Q: {qa.question}</p>
                                    <p className="text-sm text-gray-700">A: {qa.answer || "N/A"}</p>
                                  </div>
                                ))}
                            ...
                            {item.feedback && (
                              <div>
                                <p className="text-sm font-medium text-purple-600 mb-1">Feedback:</p>
                                <p className="text-sm text-gray-700 bg-yellow-50 p-3 rounded-md border border-yellow-200">{item.feedback}</p>
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {selectedCandidate.dsa && selectedCandidate.dsa.length > 0 ? (
                      selectedCandidate.dsa.map((dsa, index) => (
                        <div key={index} className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                          <div className="flex justify-between items-start mb-4">
                            <h5 className="text-lg font-medium text-gray-900">{dsa.topic?.topic} ({dsa.topic?.difficulty})</h5>
                            {dsa.score && (
                              <span className={`px-3 py-1 rounded-full text-sm font-medium ${getScoreColor(dsa.score)}`}>
                                {dsa.score.toFixed(1)}
                              </span>
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              <p className="text-sm font-medium text-purple-600 mb-1">Created At:</p>
                              <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{formatDateTime(dsa.created_at)}</p>
                            </div>
                          </div>
                        </div>
                      ))
                      ...
                      <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 text-center">
                        <FileCode className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                        <p className="text-gray-500">No DSA topics available.</p>
                      </div>
                    )}
                  </div>
                )}
                {activeTab === 'resume_conversations' && (
                  <div className="space-y-6">
                    ...
                    {selectedCandidate.resume_conversations && selectedCandidate.resume_conversations.length > 0 ? (
                      selectedCandidate.resume_conversations.map((conv, index) => (
                        <div key={index} className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                          ...
                          <div className="space-y-4">
                            <div>
                              <p className="text-sm font-medium text-purple-600 mb-1">Question:</p>
                              <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{conv.question || "N/A"}</p>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-purple-600 mb-1">Expected Answer:</p>
                              <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{conv.expected_answer || "N/A"}</p>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-purple-600 mb-1">Candidate Answer:</p>
                              <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded-md border border-gray-200">{conv.answer || "N/A"}</p>
                            </div>
                            {conv.feedback && (
                              <div>
                                <p className="text-sm font-medium text-purple-600 mb-1">Feedback:</p>
                                <p className="text-sm text-gray-700 bg-yellow-50 p-3 rounded-md border border-yellow-200">{conv.feedback}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 text-center">
                        <BarChart2 className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                        <p className="text-gray-500">No resume conversations available.</p>
                      </div>
                    )}
                  </div>
                )}
        ...
        {showApplication && selectedApplication && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto border border-gray-200 shadow-xl">
              <div className="sticky top-0 bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-4 flex justify-between items-center border-b border-gray-200">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setShowApplication(false)}
                  ...
                  <h3 className="text-lg font-semibold text-gray-900">Application Details - {selectedApplication.user?.username || "Anonymous"}</h3>
                </div>
                <button
                  onClick={() => setShowApplication(false)}
                  className="text-gray-500 hover:text-gray-700 text-xl font-bold hover:bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center"
              ...
              <div className="p-6 space-y-6">
                <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <h5 className="text-sm font-medium text-gray-900">{selectedApplication.user?.username || "Anonymous"}</h5>
                      <p className="text-xs text-gray-500">Application ID: {selectedApplication.id}</p>
                    </div>
                    <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <p className="text-sm text-gray-500 mb-1">Application Score</p>
                      <div className={`px-3 py-1 rounded-full text-sm font-bold border ${getScoreColor(selectedApplication.score)}`}>
                        {typeof selectedApplication.score === 'number' ? selectedApplication.score.toFixed(1) : "N/A"}
                      </div>
                    </div>
                    <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {selectedApplication.feedback ? (
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 text-center">
                    <BookOpen className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-500">No application feedback available.</p>
                  </div>
                )}
                <div className="bg-white rounded-lg p-6 border border-gray-200 shadow-md">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex bg-gray-200 rounded-lg p-1">
                      <button
                        onClick={() => setResumeView('extracted')}
                        className={`px-3 py-1 rounded-md text-sm font-medium ${resumeView === 'extracted' ? 'bg-purple-600 text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}
                      >
                        Extracted
                      </button>
                      <button
                        onClick={() => setResumeView('original')}
                        className={`px-3 py-1 rounded-md text-sm font-medium ${resumeView === 'original' ? 'bg-purple-600 text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}
                      >
                        Original
                      </button>
                    </div>
                  ...
                  {resumeView === 'extracted' ? (
                    (() => {
                      ...
                      if (!resumeData) {
                        return (
                          <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 text-center">
                            <User className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                            <p className="text-gray-500">No extracted resume data available.</p>
                          </div>
                        );
                      }
                      return (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          {resumeData.personalDetails.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                              ...
                              {resumeData.personalDetails.map((detail, index) => (
                                <p key={index} className="text-sm text-gray-700 mb-2">{detail}</p>
                              ))}
                            </div>
                          )}
                          {resumeData.skills.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {resumeData.skills.map((skill, index) => (
                                  <span key={index} className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs">{skill}</span>
                                ))}
                              </div>
                            </div>
                          )}
                          {resumeData.experience.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm lg:col-span-2">
                              ...
                              {resumeData.experience.map((exp, index) => (
                                <p key={index} className="text-sm text-gray-700 mb-2">{exp}</p>
                              ))}
                            </div>
                          )}
                          {resumeData.education.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                              ...
                              {resumeData.education.map((edu, index) => (
                                <p key={index} className="text-sm text-gray-700 mb-2">{edu}</p>
                              ))}
                            </div>
                          )}
                          {resumeData.certifications.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                              ...
                              {resumeData.certifications.map((cert, index) => (
                                <p key={index} className="text-sm text-gray-700 mb-2">{cert}</p>
                              ))}
                            </div>
                          )}
                          {resumeData.projects.length > 0 && (
                                ...
                                return (
                                  <div key={index} className="mb-4 p-4 bg-gray-50 rounded-md">
                                    <div className="flex items-center justify-between mb-2">
                                      <h6 className="text-sm font-bold text-gray-900">{projectName}</h6>
                                      <span className="text-xs text-gray-500">{year}</span>
                                    </div>
                                    {techStack.length > 0 && (
                                      <div className="flex flex-wrap gap-2 mb-2">
                                        {techStack.map((tech, techIndex) => (
                                          <span key={techIndex} className="px-2 py-1 bg-cyan-100 text-cyan-700 text-xs rounded-full">{tech}</span>
                                        ))}
                                      </div>
                                    )}
                                    {descriptionLines.length > 0 && (
                                      descriptionLines.map((desc, descIndex) => (
                                        <p key={descIndex} className="text-sm text-gray-700">{desc.replace(/^[-\s]+/, '').trim()}</p>
                                      ))
                                    )}
                                  </div>
                                );
                          ...
                          {resumeData.achievements.length > 0 && (
                            <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm lg:col-span-2">
                              ...
                              {resumeData.achievements.map((achievement, index) => (
                                <p key={index} className="text-sm text-gray-700 mb-2">{achievement}</p>
                              ))}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      {selectedApplication.resume ? (
                        <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
                          <div className="flex justify-between items-center mb-4">
                            <p className="text-gray-700 text-sm">View the embedded resume or download the original PDF.</p>
                            <a
                              href={selectedApplication.resume}
                              download
                              className="text-blue-600 hover:text-blue-800 flex items-center gap-1 hover:bg-blue-50 px-3 py-1 rounded-lg border border-blue-200"
                            >
                              <Download className="w-4 h-4" /> Download PDF
                            </a>
                          </div>
                          <div className="aspect-[4/5] bg-white rounded-lg border border-gray-200 overflow-hidden">
                            <iframe
                              src={`https://docs.google.com/gview?url=${encodeURIComponent(selectedApplication.resume)}&embedded=true`}
                              className="w-full h-full"
                              frameBorder="0"
